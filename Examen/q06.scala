  ...
  def loadCandidatureData(path:String):Array[Candidature]=
{
  ...
  def get_Eleve_By_ID(p_id:Int,eleves:Array[Eleve]):Eleve =
{
  return eleves.filter(_.idE==p_id)(0)
}
...
candidatures.groupBy(_.idE).map(x=>(get_Eleve_By_ID(x._1,eleves).nomE,x._2.map(y=>y.departement+">"+y.nomU).mkString(", "))).foreach(println)
